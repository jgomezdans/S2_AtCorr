...
class Kernels:
	...
	'''
        ...
        w = numpy.where(self.lt(self.vza,0))[0]
        self.vza[w] = -self.vza[w]
        self.raa[w] = self.raa[w] + numpy.pi
        w = numpy.where(self.lt(self.sza,0))[0]
    ...
    def __GetPhaang(self):
        ...
        w = numpy.where(self.lt(self.__cosphaang,-1))[0]
        self.__cosphaang[w] = -1.0
        w = numpy.where(self.gt(self.__cosphaang,1))[0]
        self.__cosphaang[w] = 1.0       
	...
	'''
        ...
        w = numpy.where(self.lt(temp,0))[0]
        temp[w] = 0.0
    ...
    def GetpAngles(self, tan1):
        ...
        '''
        t = self.BR * tan1
        w = numpy.where(self.lt( t , 0.))[0]
        t[w] = 0.0
        angp = numpy.arctan(t)
        s = numpy.sin(angp)
        c = numpy.cos(angp)
        # have to make sure c isnt 0
        w = numpy.where(self.eq(c ,0))[0]
    ...
    def GetOverlap(self):
        ...
        w = numpy.where(self.lt(self.__cost, -1))[0]
        self.__cost[w] = -1.0
        w = numpy.where(self.gt(self.__cost , 1.0))[0]
        self.__cost[w] = 1.0
        ...
        w = numpy.where(self.lt(self.__overlap , 0))[0]
